...
def _apply_static_capture_regs(com):
    # This block matches the "AM init" scripts (e.g. AmInit.py). It's expensive and
    # generally only needs to be done once per session unless something else
    # reconfigures the FPGA between captures.
...
def _apply_iq_tables_and_mode(com, iscomplx):
...
def _apply_antenna(com, antenna):
    if antenna is None:
        return
    if int(antenna) <= 0:
        return
    _ = com.consoleIo(f'rxport {int(antenna)}')
...
def main():
    ...
    if help_flag:
        ...
        print("-fs            Sampling frequency (MHz), default 150")
        print("-COMxx         (ignored) kept for compatibility")
    ...
    while True:
        if not input_file:
            ...
            try:
                ...
                # This script is socket-only. AM init / mode / tables / antenna selection
                # are expected to be handled by external setup (e.g. AmInit / LabVIEW).
                _ = SetUpDma(s)
                dma_length = getDmaLen(s)
                print(dma_length)
