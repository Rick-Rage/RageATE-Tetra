...
def plotSpectrum(segments, center, band, navg, plot_avg, plot_samples, fs):
    ...
    for seg in segments:
        ...
        try:
            fft = np.fft.fft(signal * win / N / scale * 2) / N
        except Exception as e:
...
def initHw(com, antenna, iscomplx, fs, last_state):
    """
    Minimize COM writes.

    Conventions used in this workspace:
    - fs==150 => Rev C (ignore antenna selection)
    - fs==75  => LC (antenna selection matters)
    """
    # seq off / rxdata mode are assumed to be handled by AmInit/Am_Init_REVC.
    # For LabVIEW repeatedly calling this script, avoid rewriting these each run.
    #
    # If you need this script to be self-contained again, re-enable:
    # - com.consoleIo('seq off')
    # - com.consoleIo('rxdata iq' / 'rxdata real')

    # LC only: update antenna when it changes
    if float(fs) == 75.0:
        if antenna is not None:
            ant = int(antenna)
            if ant > 0 and (last_state.get("antenna") is None or ant != int(last_state.get("antenna"))):
                com.consoleIo(f'rxport {ant}')
                last_state["antenna"] = ant
...
def main():
    ...
    try:
        ...
        firstTime = True
        hw_state = {"seq_off": False, "iscomplx": None, "antenna": None}

        try:
            while True:
                if infile is None:
                    initHw(com, antenna, iscomplx, fs, hw_state)
                    if firstTime:
                        firstTime = False
                        noiseInit(navg, com)
                    SetUpDma(IP)
