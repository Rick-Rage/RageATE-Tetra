...
RQ																			
	TEST NAME	REV-C_P-WAM_TX-PROD_10M																	
	SN	P0030																	
																			
Index	Key	Vi Sequence	##KEY : 0 = DON’T RUN ;  1= RUN w/ FP open ; -1 = RUN w/ FP closed																
0	1	Setup vi Path	C:\RageATE\Automated Tests\Tetra WAM\PRODUCTION\prod test vis\Tetra_TX-PROD_Setup.vi																
1	-1	Payload vi Path	C:\RageATE\Automated Tests\Tetra WAM\PRODUCTION\prod test vis\Tetra_TX-PROD_Payload.vi																
2	0	Close vi Path	C:\RageATE\Automated Tests\Tetra WAM\PRODUCTION\prod test vis\Tetra_TX-PROD_Close.vi																
...
7	0	Meas DC PS;Meas DCMON (Chip)	Bool;Bool	0;0	"IF = 0, Then will measure temp on : 1) 1st Iteration And 2) Every time a port change occurs.IF = 1 will every iteration"														
...
10	0	Meas S11;TXPORT	Bool;num	0;1	"if""1"" will measure s11;measure S11 only on this port number (leave blank for all)"														
...
21	0	HORN_CAL	path	C:\RageATE\Automated Tests\Tetra WAM\PRODUCTION\Test Benches\Common\Horn_Gain_121322.csv															
...
5	1	ADAR_ATT	DB	-1															
6	1	PSA_ATT	DB	-1															
7	-2	RF_FREQ_OUT	GHZ	20	40	0.01													
