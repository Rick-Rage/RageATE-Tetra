...
def ProcessTestResults(location, cursor,cnx):
    ...
    columnname, headervalues, line_count, SessionID = ReadHeader(location)
    ...
    cols = ReadWriteData(location, line_count, data, cursor,header,SessionID)
    return(cols)
...
def ReadHeader(location):
    ...
    return(columnnames, headervalues, line_count,SessionID)

#writes the majority of the data to the database
def ReadWriteData(location, line_count, data, cursor,header,SessionID):
    #gets the corresponding id from the header table
    cursor.execute(f"SELECT id from {header} where Session_ID = '{SessionID}' ORDER BY id DESC LIMIT 1;")
    #print(f"SELECT id from {header} where Session_ID = '{SessionID}' ORDER BY id DESC LIMIT 1;")
    ...
    for arg in sys.argv:
        if(arg.find(".py") != -1):
            ...
            if (filename.find("BOUNCE") != -1):
                ...
                header = "tetra_header"
            cols = ProcessTestResults(arg, cursor,cnx)
            cnx.commit()
