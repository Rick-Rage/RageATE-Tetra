...
def main(argv):
    ...
    if plotName:
        with open(plotName, "r") as fp:
            iichannel = 0
            iiloop = 0
            data = np.zeros((4,5,2048))
            for line in fp:
                (ichannel, iloop, isample, sample) = line.split(',')
                data[int(ichannel), int(iloop), int(isample)] = int(sample)
        fig, axs = plt.subplots(2, 2)
        ...
        for i,signals in enumerate(data):
            ...
            for signal in signals:
                ax.plot(list(range(305,320)), signal[305:320], '-o')
            ax.grid(True)
            ...
            for d in signal[1:]:
                ...
                d1 = d
                slope = d1 - d0
                if (d1 > 0) and (slope > mx):
                    mx = slope
                    ix = i
                i += 1
            print("mx", round(mx), "ix", ix)
        plt.show()
        return
    return
    if len(argv) > 1:
